...
usage: generate_tfrecord.py [-h] [-x JSON_DIR] [-l LABELS_PATH] [-o OUTPUT_PATH] [-i IMAGE_DIR] [-c CSV_PATH]

optional arguments:
  -h, --help            show this help message and exit
  -x JSON_DIR, --json_dir JSON_DIR
...
def json_to_csv(json_path, img_path):
    ...
    for json_file in glob.glob(json_path+'/*.json'):
        ...
        img_shape = cv2.imread(os.path.abspath(os.path.join(img_path,filename))).shape
        height = img_shape[0]
        width = img_shape[1]
        for member in d['annotation']['regions']:
           value = (filename,
                    width,
                    height,
                    member['region_attributes']['label'],
                    int(member['shape_attributes']['x']),
                    int(member['shape_attributes']['y']),
                    int(member['shape_attributes']['x']) + int(member['shape_attributes']['width']),
                    int(member['shape_attributes']['y']) + int(member['shape_attributes']['height'])
           )
...
def main(_):
    ...
    examples = json_to_csv(args.json_dir, args.image_dir)
